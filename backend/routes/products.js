...
router.post('/', protect, requireSeller, async (req, res) => {
  try {
    const { name, brand, category, subcategory, productType, sku, price, originalPrice, orig, stock, description, sizes, colors, badge, eco, icon, image, images, listed } = req.body;
...
router.put('/:id', protect, requireSeller, async (req, res) => {
  ...
  const allowed = ['name','brand','category','cat','subcategory','productType','sku','price','originalPrice','orig','stock','description','sizes','colors','badge','eco','icon','image','images','active','listed'];
